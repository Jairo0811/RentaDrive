...
final class OperationalReferenceSeeder extends Seeder
{
    ...
    {
        $company = Company::query()->where('slug', 'rentadrive-legacy')->firstOrFail();
        $branch = Branch::query()
        ...
        $tenantContext->set($company, $branch);

        try {
            Model::unguarded(function () use ($company, $branch): void {
                DB::transaction(function () use ($company, $branch): void {
                    ...
                    ])->mapWithKeys(function (array $category) use ($company): array {
                    ...
                    $toyota = VehicleBrand::query()->updateOrCreate(
                        ['company_id' => $company->getKey(), 'name' => 'Toyota'],
                        ['company_id' => $company->getKey(), 'is_active' => true],
                    );
                    $hyundai = VehicleBrand::query()->updateOrCreate(
                        ['company_id' => $company->getKey(), 'name' => 'Hyundai'],
                        ['company_id' => $company->getKey(), 'is_active' => true],
                    );
                    ...
                    ])->mapWithKeys(function (array $item) use ($company): array {
