<x-app-layout>
    <x-slot name="header">
        <div>
            <p class="text-lg font-black text-slate-950 dark:text-white">Reportes</p>
            <p class="text-xs text-slate-500">Indicadores de operación y cobro</p>
        </div>
    </x-slot>
    ...
    <x-page-header title="Reportes" subtitle="Analiza el desempeño por período y exporta las operaciones.">
        <x-slot name="actions">
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('reports.export.pdf', request()->only('from', 'to')) }}" class="btn-secondary inline-flex items-center gap-2">
                    <i class="fa-solid fa-file-pdf" aria-hidden="true"></i>
                    Exportar PDF
                </a>
                <a href="{{ route('reports.export', request()->only('from', 'to')) }}" class="btn-primary inline-flex items-center gap-2">
    ...
    <form method="GET" class="panel mb-6 grid gap-3 p-4 sm:grid-cols-[180px_180px_auto]">
        <div>
            ...
            <input id="from" type="date" name="from" value="{{ $from->format('Y-m-d') }}" class="form-input">
        </div>
        <div>
            <label class="form-label" for="to">Hasta</label>
            <input id="to" type="date" name="to" value="{{ $to->format('Y-m-d') }}" class="form-input">
        </div>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        @foreach ([
            ...
            <article class="panel p-5">
                <p class="text-xs font-bold uppercase tracking-[.14em] text-slate-400">{{ $label }}</p>
                <p class="mt-3 text-2xl font-black text-slate-950 dark:text-white">{{ $value }}</p>
    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-[1.3fr_.7fr]">
        <section class="table-shell">
            <div class="border-b border-slate-200 px-5 py-4 dark:border-slate-800">
                <h2 class="font-black text-slate-950 dark:text-white">Operaciones del período</h2>
            </div>
            @if ($rentals->isEmpty())
                <x-empty-state title="Sin operaciones" message="No hay alquileres dentro del rango elegido." />
            @else
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            @foreach ($rentals as $rental)
                                <tr>
                                    <td><a href="{{ route('rentals.show', $rental) }}" class="font-bold text-blue-600">{{ $rental->code }}</a></td>
