<x-app-layout>
            ...
            <div class="relative mt-5 flex flex-wrap gap-2">
                ...
                @can('manage rentals')
                    <a href="{{ route('rentals.create') }}" class="inline-flex items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-4 py-2.5 text-sm font-bold text-white backdrop-blur transition hover:bg-white/20">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 17h14M6 17l1-6h10l1 6M8 11l1-3h6l1 3M8 17v2m8-2v2"/></svg>
                        Abrir alquiler
                    </a>
                @endcan
            </div>
    ...
    <section class="mt-6 grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        <article class="panel p-5 sm:p-6">
            ...
            <div class="mt-6 h-72"><canvas id="incomeChart" data-labels='@json($monthlyIncome->pluck("label"))' data-values='@json($monthlyIncome->pluck("value"))'></canvas></div>
        </article>
        ...
        <article class="panel p-5 sm:p-6">
            ...
            <div class="mt-6 h-72"><canvas id="fleetChart" data-labels='@json(["Disponible", "Reservado", "Alquilado", "Mantenimiento", "Inactivo"])' data-values='@json([($fleetStatus["available"] ?? 0), ($fleetStatus["reserved"] ?? 0), ($fleetStatus["rented"] ?? 0), ($fleetStatus["maintenance"] ?? 0), ($fleetStatus["inactive"] ?? 0)])'></canvas></div>
        </article>
    </section>
    ...
    <section class="mt-6 panel overflow-hidden">
        <div class="border-b border-slate-200 px-5 py-4 dark:border-slate-800 sm:px-6">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div><h2 class="font-bold text-slate-950 dark:text-white">Calendario de reservas</h2><p class="mt-1 text-sm text-slate-500">{{ $calendar['title'] }}</p></div>
        ...
        <div class="overflow-x-auto p-4 sm:p-6">
            <div class="calendar-grid min-w-[720px]">
                @foreach (['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'] as $weekday)<div class="calendar-weekday">{{ $weekday }}</div>@endforeach
                @for ($blank = 1; $blank < $calendar['firstWeekday']; $blank++)<div class="calendar-day calendar-day-muted"></div>@endfor
                @for ($day = 1; $day <= $calendar['daysInMonth']; $day++)
                    @php($events = $calendar['reservations']->where('day', $day))
                    <div class="calendar-day {{ now()->day === $day ? 'calendar-day-today' : '' }}">
                        <span class="calendar-number">{{ $day }}</span>
                        <div class="mt-2 space-y-1">
                            @foreach ($events->take(2) as $event)
                                <a href="{{ $event['url'] }}" class="calendar-event" title="{{ $event['customer'] }} · {{ $event['vehicle'] }}"><span>{{ $event['time'] }}</span><strong>{{ $event['customer'] }}</strong></a>
                            @endforeach
                            @if ($events->count() > 2)<span class="text-[10px] font-bold text-blue-500">+{{ $events->count() - 2 }} más</span>@endif
                        </div>
                    </div>
                @endfor
            </div>
    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-2">
        <section class="panel overflow-hidden">
            <div class="border-b border-slate-200 px-5 py-4 dark:border-slate-800 sm:px-6"><h2 class="flex items-center gap-2 font-bold text-slate-950 dark:text-white"><svg class="h-5 w-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3M5 11h14M5 5h14a1 1 0 0 1 1 1v14H4V6a1 1 0 0 1 1-1Z"/></svg>Próximas reservas</h2><p class="mt-1 text-sm text-slate-500">Entregas pendientes y confirmadas.</p></div>
            <div class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse ($upcomingReservations as $reservation)
                    <a href="{{ route('reservations.show', $reservation) }}" class="flex items-center justify-between gap-4 px-5 py-4 transition hover:bg-slate-50 dark:hover:bg-slate-800/50 sm:px-6"><div><p class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ $reservation->customer->full_name }}</p><p class="mt-1 text-xs text-slate-500">{{ $reservation->code }} · {{ $reservation->vehicle?->display_name ?? $reservation->category->name }}</p></div><div class="text-right"><p class="text-sm font-semibold text-slate-700 dark:text-slate-200">{{ $reservation->start_at->format('d/m/Y') }}</p><p class="text-xs text-slate-500">{{ $reservation->start_at->format('h:i A') }}</p></div></a>
                @empty
        ...
        <section class="panel overflow-hidden">
            <div class="border-b border-slate-200 px-5 py-4 dark:border-slate-800 sm:px-6"><h2 class="flex items-center gap-2 font-bold text-slate-950 dark:text-white"><svg class="h-5 w-5 text-violet-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 17h14M6 17l1-6h10l1 6M8 11l1-3h6l1 3M8 17v2m8-2v2"/></svg>Alquileres activos</h2><p class="mt-1 text-sm text-slate-500">Unidades actualmente fuera.</p></div>
            <div class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse ($activeRentals as $rental)
                    <a href="{{ route('rentals.show', $rental) }}" class="flex items-center justify-between gap-4 px-5 py-4 transition hover:bg-slate-50 dark:hover:bg-slate-800/50 sm:px-6"><div><p class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ $rental->vehicle->display_name }}</p><p class="mt-1 text-xs text-slate-500">{{ $rental->customer->full_name }} · {{ $rental->code }}</p></div><div class="text-right"><p class="text-xs font-bold uppercase tracking-wide text-slate-400">Retorno</p><p class="mt-1 text-sm font-semibold {{ $rental->expected_return_at->isPast() ? 'text-red-500' : 'text-slate-700 dark:text-slate-200' }}">{{ $rental->expected_return_at->format('d/m h:i A') }}</p></div></a>
                @empty
